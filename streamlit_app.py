...
def compute(month: str) -> dict:
    ...
    for t in rows:
        if t["kind"] == "expense" and t.get("cat_id"):
    ...
    for t in rows:
        if t["kind"] == "income" and t.get("cat_id"):
            spent[t["cat_id"]] = spent.get(t["cat_id"], 0) + t["amount"]
    ...
    return {"rows": rows, "ins": ins, "common": common_out, "people": people,
...
with tab_dash:
    ...
    for col, p in ((c1, pa), (c3, ps)):
        col.markdown(
            ...
            f'<div class="row"><span>Spent</span><span>{money(p["actual"])}</span></div>'
        ...
        for c in cats:
            ...
            if c["flow"] == "in":
                new.append({"id": store_mod.new_id(), "date": f"{month}-01", "kind": "income",
                            "cat_id": c["id"], "who": c["owner"], "amount": c["budget"], "note": c["name"]})
            else:
                new.append({"id": store_mod.new_id(), "date": f"{month}-01", "kind": "expense",
                            "cat_id": c["id"], "who": "", "amount": c["budget"], "note": "Fixed bill"})
        if not new:
...
with tab_ledger:
    ...
    kind = st.radio("Entry type", ["Expense", "Income"], horizontal=True, label_visibility="collapsed")

    with st.form("add_entry", clear_on_submit=True):
        c1, c2, c3, c4 = st.columns([1.1, 2, 1, 1.6])
        date = c1.date_input("Date", value=dt.date(y, mo, default_day))

        if kind == "Expense":
            pick = c2.selectbox("Category", out_cats,
                                format_func=lambda c: f"{c['name']} · {NAME.get(c['owner'], c['owner'])}")
        else:
            options = [("cat", c) for c in in_cats] + [("who", p) for p in PEOPLE] + [("who", {"id": "common", "name": "Shared"})]
            pick = c2.selectbox("Source", options,
        ...
        note = c4.text_input("Note", placeholder="Optional")

        if st.form_submit_button(f"Add {kind.lower()}", type="primary") and amount > 0:
            if kind == "Expense":
                row = {"id": store_mod.new_id(), "date": str(date), "kind": "expense",
                       "cat_id": pick["id"], "who": "", "amount": amount, "note": note}
            elif pick[0] == "cat":
                c = pick[1]
                row = {"id": store_mod.new_id(), "date": str(date), "kind": "income",
                       "cat_id": c["id"], "who": c["owner"], "amount": amount, "note": note or c["name"]}
            else:
                row = {"id": store_mod.new_id(), "date": str(date), "kind": "income",
                       "cat_id": "", "who": pick[1]["id"], "amount": amount, "note": note or "Extra income"}
            store.add_txns([row])
    ...
    for t in rows:
        c1, c2, c3, c4 = st.columns([1, 5, 1.2, 0.6])
        ...
        c3.markdown(f'<span style="font-family:ui-monospace;font-size:.85rem;'
                    f'color:{GOOD if t["kind"] == "income" else INK}">{sign}{money(t["amount"])}</span>',
                    unsafe_allow_html=True)
        if c4.button("✕", key=f"del{t['id']}", help="Delete entry"):
...
with tab_budget:
    ...
    st.caption("Direction decides the maths: money out counts against the budget, "
               "money in adds to income. Fixed means the same amount every month.")

    df = pd.DataFrame(cats)[["id", "name", "owner", "flow", "type", "budget", "fixed"]]
